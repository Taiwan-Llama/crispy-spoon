...
class DuckDuckGoSearchServer {
  ...
  private searchEndpoint = 'https://html.duckduckgo.com/html/';
  ...
  private async handleWebSearch(request: any) {
  ...
  private async handleSummarizeSearch(request: any) {
    const query = String(request.params.arguments?.query);
  ...
  private generateSearchSummary(results: SearchResult[]): string {
    if (results.length === 0) {
